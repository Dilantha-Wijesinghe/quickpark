...
const rootDomain = 'quickpark.com'

export const createAuthOptions = (appName?: string): NextAuthOptions => {
  // Create unique cookie name for each app in development
  const cookiePrefix =
    appName && process.env.NODE_ENV === 'development' ? `${appName}-` : ''
...
// Default authOptions for backward compatibility
